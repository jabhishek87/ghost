...
class FaceEmbed(TensorDataset):
    ...
    def __getitem__(self, item):
        ...
        if secrets.SystemRandom().random() > self.same_prob:
            image_path = secrets.choice(self.datasets[secrets.SystemRandom().randint(0, len(self.datasets)-1)])
...
class FaceEmbedVGG2(TensorDataset):
    ...
    def __getitem__(self, item):
        ...
        if secrets.SystemRandom().random() > self.same_prob:
            image_path = secrets.choice(self.images_list)
            Xt = cv2.imread(image_path)[:, :, ::-1]
            Xt = Image.fromarray(Xt)
            same_person = 0
        else:
            if self.same_identity:
                image_path = secrets.choice(self.folder2imgs[folder_name])
                Xt = cv2.imread(image_path)[:, :, ::-1]
                Xt = Image.fromarray(Xt)
            else:
                Xt = Xs.copy()
